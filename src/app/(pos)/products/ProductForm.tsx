    ...
    <>
      ...
      <form ref={formRef} action={createProductAction} className="mt-4 grid gap-2 sm:grid-cols-7">
        ...
        <label className="sm:col-span-2">
          ...
          <select name="unitId" className="field" required defaultValue={(units.find((u) => u.name.toLowerCase() === "piece") ?? units[0])?.id ?? ""}>
            {units.map((u) => (
            ...
            {preview ? (
              <div className="relative">
                <img
                  src={preview}
                  alt="Preview"
                  className="h-16 w-28 rounded-lg object-cover border border-slate-200"
                />
                <button
                  type="button"
